...
export const m365SideloadingCommand: CLICommand = {
  ...
  examples: [
    ...
    },
    {
      ...
      description:
        "Sideload a declarative agent application package with JSON-based manifest to Teams, Outlook, and the Microsoft 365 app.",
    },
    {
  ...
  handler: async (ctx) => {
    ...
    const scope = ctx.optionValues["scope"] as string;
    ...
    if (ctx.optionValues["file-path"] !== undefined) {
      const res = await packageService.sideLoading(tokenAndUpn[0], manifestPath, scope as AppScope);
      if (res[2]) {
